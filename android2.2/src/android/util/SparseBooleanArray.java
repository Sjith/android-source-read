...
/**
 * SparseBooleanArrays map integers to booleans. Unlike a normal array of
 * booleans there can be gaps in the indices. It is intended to be more
 * efficient than using a HashMap to map Integers to Booleans.
 * <br>从integer到boolean的映射数组 
 */
public class SparseBooleanArray {
	...
	/**
	 * Gets the boolean mapped from the specified key, or <code>false</code> if
	...
	/**
	 * Gets the boolean mapped from the specified key, or the specified value if
	 * no such mapping has been made.
	 */
	 ...
	 * <br>删除一个值，如果存在则将折半查找获得的位置后的数据向前移动，同时减小mSize
	 */
	...
	public void put(int key, boolean value) {
		...
		if (i >= 0) {
			...
			if (mSize >= mKeys.length) {
				...
				// Log.e("SparseBooleanArray", "grow " + mKeys.length + " to " +
				// n);

 ...
 * arbitrary objects into a Parcel) is designed as a high-performance IPC
 * transport. As such, it is not appropriate to place any Parcel data in to
 * persistent storage: changes in the underlying implementation of any of the
 ...
 * various types. There are six major classes of such functions available.
 * </p>
 * 
 * <h3>Primitives</h3>
 * 
 * <p>
 * The most basic data functions are for writing and reading primitive data
 * types: {@link #writeByte}, {@link #readByte}, {@link #writeDouble},
 * {@link #readDouble}, {@link #writeFloat}, {@link #readFloat},
 * {@link #writeInt}, {@link #readInt}, {@link #writeLong}, {@link #readLong},
 * {@link #writeString}, {@link #readString}. Most other data operations are
 * built on top of these. The given data is written and read using the endianess
 * of the host CPU.
 * </p>
 * 
 ...
 * 
 * <p>
 * There are a variety of methods for reading and writing raw arrays of
 * primitive objects, which generally result in writing a 4-byte length followed
 * by the primitive data items. The methods for reading can either read the data
 * into an existing array, or create and return a new array. These available
 * types are:
 * </p>
 * 
 * <ul>
 ...
 * You can use the direct methods {@link #writeParcelable(Parcelable, int)} and
 * {@link #readParcelable(ClassLoader)} or {@link #writeParcelableArray} and
 * {@link #readParcelableArray(ClassLoader)} to write or read. These methods
 * write both the class type and its data to the Parcel, allowing that class to
 ...
 * Parcelables: {@link #writeTypedArray}, {@link #writeTypedList(List)},
 * {@link #readTypedArray} and {@link #readTypedList}. These methods do not
 * write the class information of the original object: instead, the caller of
 * the read function must know what type to expect and pass in the appropriate
 * {@link Parcelable.Creator Parcelable.Creator} instead to properly construct
 * the new object and read its data. (To more efficient write and read a single
 * Parceable object, you can directly call {@link Parcelable#writeToParcel
 * Parcelable.writeToParcel} and {@link Parcelable.Creator#createFromParcel
 ...
 * 
 * <p>
 * A special type-safe container, called {@link Bundle}, is available for
 ...
 * contents into a Parcel. The methods to use are {@link #writeBundle(Bundle)},
 ...
 * protocol with a Binder object. Any such interface can be written in to a
 * Parcel, and upon reading you will receive either the original object
 ...
 * {@link #createBinderArray()}, {@link #writeBinderList(List)},
 * {@link #readBinderList(List)}, {@link #createBinderArrayList()}.
 * </p>
 * 
 * <p>
 ...
 * original file descriptor: the object and fd is different, but operating on
 * the same underlying file stream, with the same position, etc. The methods to
 * use are {@link #writeFileDescriptor(FileDescriptor)},
 ...
 * of arbitrary types. These all revolve around the {@link #writeValue(Object)}
 * and {@link #readValue(ClassLoader)} methods which define the types of objects
 * allowed. The container methods are {@link #writeArray(Object[])},
 ...
 * {@link #writeSparseArray(SparseArray)}, {@link #readSparseArray(ClassLoader)}.
 */
public final class Parcel {
	 ...
	 * {@link #dataSize}-{@link #dataPosition}.
	 */
	 ...
	 * {@link #dataSize}. The difference between it and dataSize() is the amount
	 * of room left until the parcel needs to re-allocate its data buffer.
	 */
	public final native int dataCapacity();

	/**
	 * Change the amount of data in the parcel. Can be either smaller or larger
	 * than the current size. If larger than the current capacity, more memory
	 * will be allocated.
	 * 
	 * @param size
	 ...
	 * 
	 * @param size
	 *            The new capacity of the parcel, in bytes. Can not be less than
	 *            {@link #dataSize} -- that is, you can not drop existing data
	 *            with this method.
	 */
	 ...
	 * 
	 * <p class="note">
	 * The data you retrieve here <strong>must not</strong> be placed in any
	 * kind of persistent storage (on local disk, across a network, etc). For
	 * that, you should use standard serialization or another kind of general
	 * serialization mechanism. The Parcel marshalled representation is highly
	 * optimized for local IPC, and as such does not attempt to maintain
	 * compatibility with data created in different versions of the platform.
	 */
	 ...
	 * at the current dataPosition(), growing dataCapacity() if needed. The Map
	 * keys must be String objects. The Map values are written using
	 * {@link #writeValue} and must follow the specification there.
	 * 
	 * <p>
	 * It is strongly recommended to use {@link #writeBundle} instead of this
	 * method, since the Bundle class provides a type-safe API that allows you
	 * to avoid mysterious type errors at the point of marshalling.
	 */
	...
	/**
	 * Flatten a Map into the parcel at the current dataPosition(), growing
	 * dataCapacity() if needed. The Map keys must be String objects.
	 */
	 ...
	 * dataPosition(), growing dataCapacity() if needed. The SparseArray values
	 * are written using {@link #writeValue} and must follow the specification
	 * there.
	 */
	 ...
	 * the current dataPosition() and growing dataCapacity() if needed. The type
	 * of the objects in the list must be one that implements Parcelable. Unlike
	 * the generic writeList() method, however, only the raw data of the objects
	 * is written and not their type, so you must use the corresponding
	 ...
	 * dataPosition() and growing dataCapacity() if needed. They can later be
	 * retrieved with {@link #createBinderArrayList} or {@link #readBinderList}.
	 * 
	 * @param val
	 *            The list of strings to be written.
	 * 
	 ...
	 * the parcel, at the current dataPosition() and growing dataCapacity() if
	 * needed. The type of the objects in the array must be one that implements
	 * Parcelable. Unlike the {@link #writeParcelableArray} method, however,
	 * only the raw data of the objects is written and not their type, so you
	 * must use {@link #readTypedArray} with the correct corresponding
	 ...
	 * {@link #writeSerializable} for caveats). Note that all of the previous
	 * types have relatively efficient implementations for writing to a Parcel;
	 * having to rely on the generic serialization approach is much less
	 ...
	 * recommended that this method be avoided, since the serialization overhead
	 * is extremely large, and this approach will be much slower than using the
	 * other approaches to writing data in to a Parcel.
	 */
	 ...
	 * parcel, to be used when returning an exception from a transaction. Note
	 * that this currently only supports a few exception types; any other
	 * exception will be re-thrown by this function as a RuntimeException (to be
	 * caught by the system's last-resort exception handling when dispatching a
	 * transaction).
	 * 
	 * <p>
	 ...
	 * parcel, to be used after receiving the result of a transaction. This will
	 * throw the exception for you if it had been written to the Parcel,
	...
	/**
	 * Use this function for customized exception handling. customized method
	 * call this method for all unknown case
	 * 
	 * @param code
	 *            exception code
	 * @param msg
	...
	public final void readException(int code, String msg) {
		...
		throw new RuntimeException("Unknown exception code: " + code + " msg "
				+ msg);
	}
	 ...
	 * loader of the Bundle for later retrieval of Parcelable objects. Returns
	 ...
	 * dataPosition(). Returns null if the previously written array was null.
	 ...
	 * the parcel that was written with {@link #writeTypedList} at the current
	 * dataPosition(). Returns null if the previously written list object was
	 * null. The list <em>must</em> have previously been written via
	 * {@link #writeTypedList} with the same object type.
	 * 
	...
	/**
	 * Read into the given List items containing a particular object type that
	 * were written with {@link #writeTypedList} at the current dataPosition().
	 * The list <em>must</em> have previously been written via
	 * {@link #writeTypedList} with the same object type.
	 * 
	 * @return A newly created ArrayList containing objects with the same data
	 *         as those that were previously written.
	 ...
	 * that was written with {@link #writeStringList} at the current
	 * dataPosition(). Returns null if the previously written list object was
	 * null.
	 * 
	 ...
	 * parcel that was written with {@link #writeBinderList} at the current
	 * dataPosition(). Returns null if the previously written list object was
	 * null.
	 * 
	 * @return A newly created ArrayList containing strings with the same data
	...
	/**
	 * Read and return a new array containing a particular object type from the
	 * parcel at the current dataPosition(). Returns null if the previously
	 * written array was null. The array <em>must</em> have previously been
	 * written via {@link #writeTypedArray} with the same object type.
	 * 
	 * @return A newly created array containing objects with the same data as
	...
	/**
	 * Write a heterogeneous array of Parcelable objects into the Parcel. Each
	 * object in the array is written along with its class name, so that the
	 * correct class can later be instantiated. As a result, this has
	 * significantly more overhead than {@link #writeTypedArray}, but will
	 ...
	 * 
	 * @param loader
	 *            A ClassLoader from which to instantiate the Parcelable object,
	 *            or null for the default class loader.
	 * @return Returns the newly created Parcelable, or null if a null object
	 *         has been written.
	 * @throws BadParcelableException
	 *             Throws BadParcelableException if there was an error trying to
	 *             instantiate the Parcelable.
	 */
	...
	public final Serializable readSerializable() {
		...
		if (name == null) {
			...
			// is nothing left in the Parcel to read, or the next value wasn't a
			// String), so
			// return null, which indicates that the name wasn't found in the
			// parcel.
			return null;
		}
	...
	private Parcel(int obj) {
		...
		// Log.i("Parcel", "Initializing obj=0x" + Integer.toHexString(obj),
		// mStack);
		init(obj);
	}

 ...
 * {@link AccountManager} call. Methods are provided to check if the computation
 * is complete, to wait for its completion, and to retrieve the result of the
 * computation. The result can only be retrieved using method <tt>get</tt> when
 * the computation has completed, blocking if necessary until it is ready.
 * Cancellation is performed by the <tt>cancel</tt> method. Additional methods
 * are provided to determine if the task completed normally or was cancelled.
 * Once a computation has completed, the computation cannot be cancelled. If you
 * would like to use a <tt>Future</tt> for the sake of cancellability but not
 * provide a usable result, you can declare types of the form
 * <tt>Future&lt;?&gt;</tt> and return <tt>null</tt> as a result of the
 * underlying task.
 */
public interface AccountManagerFuture<V> {
	...
	/**
	 * Accessor for the future result the {@link AccountManagerFuture}
	 * represents. This call will block until the result is available. In order
	 * to check if the result is available without blocking, one may call
	 ...
	 * {@link #isDone()} and {@link #isCancelled()}. If the request that
	 * generated this result fails or is canceled then an exception will be
	 * thrown rather than the call returning normally. If a timeout is specified
	 * then the request will automatically be canceled if it does not complete

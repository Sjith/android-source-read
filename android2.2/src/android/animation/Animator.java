...
public abstract class Animator implements Cloneable {
	...
	/**
	 * 开始动画，如果startDelay非零，则将会延迟一段时间，本方法执行后悔调用监控器的onAnimationStart方法。 <br>
	 ...
	 * calls to {@link AnimatorListener#onAnimationStart(Animator)} for any
	 * listeners of this animator.
	 * 
	 * <p>
	...
	/**
	 * 获取动画在调用了{@link #start()}之后需要延迟的时间。 <br>
	...
	/**
	 * 设置用于计算动画的失去因子的interpolator。interpolator据顶了动画是否执行一个线性或非线性的运动，例如加速 ，减速,默认值为
	 * {@link android.view.animation.AccelerateDecelerateInterpolator} <br>
	...
	/**
	 * Returns whether this Animator has been started and not yet ended. This
	 * state is a superset of the state of {@link #isRunning()}, because an
	 * Animator with a nonzero {@link #getStartDelay() startDelay} will return
	 * true for {@link #isStarted()} during the delay phase, whereas
	 * {@link #isRunning()} will return true only after the delay phase is
	 * complete.
	 * 
	 ...
	 * 
	 * @param listener
	 *            the listener to be removed from the current set of listeners
	 *            for this animation.
	 */
	...
	/**
	 * Gets the set of {@link android.animation.Animator.AnimatorListener}
	 * objects that are currently listening for events on this
	 * <code>Animator</code> object.
	 * 
	 ...
	 * <code>getListeners()</code> followed by calling <code>clear()</code> on
	 * the returned list of listeners.
	 */
	 ...
	 * properties. An ValueAnimator object will ignore the request since it does
	 ...
	 * Not all subclasses operate on target objects (for example,
	 * {@link ValueAnimator}, but this method is on the superclass for the
	 * convenience of dealing generically with those subclasses that do handle
	 * targets.
	 * 
	 * @param target
	...
	/**
	 * AnimatorListener用于接收animation通知信息，通知信息表示了与动画相关的事件，例如动画结束或者动画重复。 <br>
	 * <p>
	...
	public static interface AnimatorListener {
		/**
		 * <p>
		 * Notifies the start of the animation. 动画开始
		 * </p>
		 * 
		 * @param animation
		...
		/**
		 * <p>
		 * 动画结束 Notifies the end of the animation. This callback is not invoked
		...
		/**
		 * <p>
		 * 动画取消 Notifies the cancellation of the animation. This callback is not
		...
		/**
		 * <p>
		 * 动画重复 Notifies the repetition of the animation.
		 * </p>
		 * 
		 * @param animation

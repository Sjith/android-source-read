...
/**
 * Drawable是"能够被画的"普通抽象。大部分情况下将Drawable作为从resource中获取的用于显示到屏幕的资源。
 *  <br>
 ...
 * 
 * <li>The {@link #getPadding} method can return from some Drawables information
 * about how to frame content that is placed inside of them. For example, a
 * Drawable that is intended to be the frame for a button widget would need to
 * return padding that correctly places the label inside of itself.
 * 
 ...
 * 
 * <li>A Drawable can perform animations by calling back to its client through
 * the {@link Callback} interface. All clients should support this interface
 * (via {@link #setCallback}) so that animations will work. A simple way to do
 * this is through the system facilities such as
 ...
 * <li><b>Shape</b>: contains simple drawing commands instead of a raw bitmap,
 ...
 * <li><b>States</b>: a compound drawable that selects one of a set of drawables
 * based on its state.
 * <li><b>Levels</b>: a compound drawable that selects one of a set of drawables
 * based on its level.
 * <li><b>Scale</b>: a compound drawable with a single child drawable, whose
 * overall size is modified based on the current level.
 * </ul>
 * <p>
 * For information and examples of creating drawable resources (XML or bitmap
 * files that can be loaded in code), see <a href="{@docRoot}
 * guide/topics/resources/resources-i18n.html">Resources and
...
public abstract class Drawable {
	 ...
	 * copyBounds(rect) instead. You should also not change the object returned
	...
	/**
	 * Set a mask of the configuration parameters for which this drawable may
	 * change, requiring that it be re-created.
	 * 
	 * @param configs
	...
	/**
	 * Return a mask of the configuration parameters for which this drawable mau
	 * change, requiring that it be re-created. The default implementation
	 * returns whatever was provided through
	 * {@link #setChangingConfigurations(int)} or 0 by default. Subclasses may
	 * extend this to or in the changing configurations of any other drawables
	 * they hold.
	 * 
	 ...
	 * extends {@link android.graphics.drawable.Drawable Drawable}. Upon
	 * retrieving a drawable, use
	...
	public static interface Callback {
		 ...
		 * with {@link #scheduleDrawable}. An implementation can generally
		 * simply call
		 * {@link android.os.Handler#removeCallbacks(Runnable, Object)} with the
		 * parameters <var>(what, who)</var> to unschedule the drawable.
		 * 
		 * @param who
	 ...
	 * redrawn. Does nothing if there is no Callback attached to the Drawable.
	 * 
	...
	/**
	 * Use the current {@link Callback} implementation to have this Drawable
	 * scheduled. Does nothing if there is no Callback attached to the Drawable.
	 * 
	 * @param what
	 ...
	 * see the relevant documentation. As an example, the background for widgets
	 * like Button understand the following states: [
	 * {@link android.R.attr#state_focused},
	 * {@link android.R.attr#state_pressed}].
	 * 
	 * <p>
	 * If the new state you are supplying causes the appearance of the Drawable
	 * to change, then it is responsible for calling {@link #invalidateSelf} in
	 * order to have itself redrawn, <em>and</em> true will be returned from
	 * this function.
	 * 
	 * <p>
	 * Note: The Drawable holds a reference on to <var>stateSet</var> until a
	 * new state array is given to it, so you must not modify this array during
	 * that time.
	 * </p>
	 * 
	 * @param stateSet
	 ...
	 * 
	 * @return Returns true if this change in state has caused the appearance of
	 *         the Drawable to change (hence requiring an invalidate), otherwise
	 *         returns false.
	 */
	 ...
	 * {@link android.R.attr#state_focused},
	 * {@link android.R.attr#state_selected}, etc. Some drawables may modify
	...
	/**
	 * @return The current drawable that will be used by this drawable. For
	 *         simple drawables, this is just the drawable itself. For drawables
	 *         that change state like {@link StateListDrawable} and
	 *         {@link LevelListDrawable} this will be the child drawable
	 *         currently in use.
	 */
	 ...
	 * 
	 * <p>
	 * If the new level you are supplying causes the appearance of the Drawable
	 * to change, then it is responsible for calling {@link #invalidateSelf} in
	 * order to have itself redrawn, <em>and</em> true will be returned from
	 * this function.
	 * 
	 ...
	 * 
	 * @return Returns true if this change in level has caused the appearance of
	 *         the Drawable to change (hence requiring an invalidate), otherwise
	 *         returns false.
	 */
	...
	/**
	 * Set whether this Drawable is visible. This generally does not impact the
	 * Drawable's behavior, but is a hint that can be used by some Drawables,
	 * for example, to decide whether run animations.
	 * 
	 * @param visible
	 *            Set to true if visible, false if not.
	 * @param restart
	 *            You can supply true here to force the drawable to behave as if
	 *            it has just become visible, even if it had last been set
	 *            visible. Used for example to force animations to restart.
	 * 
	 * @return boolean Returns true if the new visibility is different than its
	 *         previous state.
	 */
	 ...
	 * 
	 * <p>
	 * Generally a Drawable should be as conservative as possible with the value
	 * it returns. For example, if it contains multiple child drawables and only
	 * shows one of them at a time, if only one of the children is TRANSLUCENT
	 * and the others are OPAQUE then TRANSLUCENT should be returned. You can
	 * use the method {@link #resolveOpacity} to perform a standard reduction of
	 * two opacities to the appropriate single output.
	 * 
	 * <p>
	 * Note that the returned value does <em>not</em> take into account a custom
	 * alpha or color filter that has been applied by the client through the
	...
	/**
	 * Return the appropriate opacity value for two source opacities. If either
	 * is UNKNOWN, that is returned; else, if either is TRANSLUCENT, that is
	 * returned; else, if either is TRANSPARENT, that is returned; else, OPAQUE
	 * is returned.
	 * 
	 * <p>
	 ...
	 *         report, else a Region holding the parts of the Drawable's bounds
	 ...
	 * specified state.
	 * 
	 * @return Returns true if the state change has caused the appearance of the
	 *         Drawable to change (that is, it needs to be drawn), else false if
	 *         it looks the same and there is no need to redraw it since its
	 *         last state.
	 */
	 ...
	 * level.
	 * 
	 * @return Returns true if the level change has caused the appearance of the
	 *         Drawable to change (that is, it needs to be drawn), else false if
	 *         it looks the same and there is no need to redraw it since its
	 *         last level.
	 */
	...
	public static Drawable createFromResourceStream(Resources res,
			TypedValue value, InputStream is, String srcName) {
	...
	public static Drawable createFromResourceStream(Resources res,
		...
		/*
		 * ugh. The decodeStream contract is that we have already allocated the
		 * pad rect, but if the bitmap does not had a ninepatch chunk, then the
		 * pad will be ignored. If we could change this to lazily alloc/assign
		 * the rect, we could avoid the GC churn of making new Rects only to
		...
		Bitmap bm = BitmapFactory.decodeResourceStream(res, value, is, pad,
				opts);
		if (bm != null) {
	 ...
	 * create resources in XML, see <a href="{@docRoot}
	 * guide/topics/resources/resources-i18n.html">Resources and
	 * Internationalization</a>.
	 */
	public static Drawable createFromXml(Resources r, XmlPullParser parser)
		...
		if (drawable == null) {
			throw new RuntimeException("Unknown initial tag: "
					+ parser.getName());
		}
	 ...
	 * tag in an XML document, tries to create a Drawable from that tag. Returns
	 * null if the tag is not a valid drawable.
	 */
	...
	public void inflate(Resources r, XmlPullParser parser, AttributeSet attrs)
		...
		TypedArray a = r.obtainAttributes(attrs,
				com.android.internal.R.styleable.Drawable);
		inflateWithAttributes(r, parser, a,
				com.android.internal.R.styleable.Drawable_visible);
		a.recycle();
	}

	void inflateWithAttributes(Resources r, XmlPullParser parser,
			TypedArray attrs, int visibleAttr) throws XmlPullParserException,
			IOException {
	...
	public static abstract class ConstantState {
		/**
		 * Create a new drawable without supplying resources the caller is
		 * running in. Note that using this means the density-dependent
		 ...
		 * implemented for drawables that change based on the target density of
		 * their caller (that is depending on whether it is in compatibility
		 * mode).
		 */
		...
		/**
		 * Return a bit mask of configuration changes that will impact this
		 * drawable (and thus require completely reloading it).
		 */
	...
	private static Drawable drawableFromBitmap(Resources res, Bitmap bm,
			byte[] np, Rect pad, String srcName) {

...
public class CursorWindow extends SQLiteClosable implements Parcelable {
	/** The pointer to the native window class,native window类的指针 */
	...
	/**
	 * 返回本window在整个cursor数据集的开始位置
	 * <br>
	 * Returns the starting position of this window within the entire Cursor's
	 * result set.
	 * 
	 * @return the starting position of this window within the entire Cursor's
	 *         result set.
	 */
	 ...
	 * 
	 * @param row
	 *            the row to read from, row - getStartPosition() being the
	 *            actual row in the window
	 * @param col
	 ...
	 *            the column to read from
	 * @param buffer
	 *            the CharArrayBuffer to copy the text into, If the requested
	 *            string is larger than the buffer a new char buffer will be
	 *            created to hold the string. and assigne to
	 *            CharArrayBuffer.data
	 */
	public void copyStringToBuffer(int row, int col, CharArrayBuffer buffer) {
		if (buffer == null) {
			throw new IllegalArgumentException(
					"CharArrayBuffer should not be null");
		}
		if (buffer.data == null) {
		...
		acquireReference();
		try {
			char[] newbuf = copyStringToBuffer_native(row - mStartPos, col,
					buffer.data.length, buffer);
			if (newbuf != null) {
				buffer.data = newbuf;
			}
		} finally {
	...
	private native char[] copyStringToBuffer_native(int row, int col,
			int bufferSize, CharArrayBuffer buffer);
	...
	/**
	 * Does the native side initialization with an existing binder from another
	 * process
	 */

 ...
 * by a database query. <br>
 * 提供对数据库查询结果集合的随机读写操作
 */
public interface Cursor {
	/**
	 * Returns the numbers of rows in the cursor. <br>
	 * 返回行数
	 * 
	...
	/**
	 * 当前所在的行号，以0为基础，如果是首次返回则为-1，-1代表在首行前面。 <br>
	...
	/**
	 * 移动cursor相对数目的行数。正参数代表向前移动，负参数代表向后移动。如果参数代表的行是可到达的则返回true。 <br>
	...
	/**
	 * 移动cursor到某一个绝对位置，有效范围是-1和count之间。 <br>
	 ...
	 * @hide
	 * @deprecated use the {@link ContentResolver} update methods instead of the
	...
	/**
	 * Returns <code>true</code> if there are pending updates that have not yet
	 * been committed.
	 * 
	 * @return <code>true</code> if there are pending updates that have not yet
	 *         been committed.
	 * @hide
	 * @deprecated use the {@link ContentResolver} update methods instead of the
	 *             Cursor update methods
	...
	/**
	 * Updates the value for the given column in the row the cursor is currently
	 * pointing at. Updates are not committed to the backing store until
	...
	/**
	 * Removes the value for the given column in the row the cursor is currently
	 * pointing at. Updates are not committed to the backing store until
	 * {@link #commitUpdates()} is called.
	 * 
	 * @param columnIndex
	 ...
	 * updates included in values. After completion, this method leaves the data
	 * in an inconsistent state and you should call {@link #requery} before
	 * reading data from the cursor again.
	 * 
	 * @param values
	 *            A map from row IDs to Maps associating column names with
	...
	/**
	 * 注册一个observer，当cursor对应的内容发生改变的时候这个observer被调用。通常情况下直到{@link #query()}
	 * 被调用数据集才能变化。 <br>
	 ...
	 * 
	 * @return cursor-defined values, or Bundle.EMTPY if there are no values.
	 *         Never null.
	 */
	Bundle getExtras();

...
public class KeyCharacterMap {
	...
	/**
	 * This private-use character is used to trigger Unicode character input by
	 * hex digits.
	 */
	...
	private static SparseArray<WeakReference<KeyCharacterMap>> sInstances = new SparseArray<WeakReference<KeyCharacterMap>>();
	...
	public static KeyCharacterMap load(int keyboard) {
		synchronized (sLock) {
			...
			sInstances
					.put(keyboard, new WeakReference<KeyCharacterMap>(result));
			return result;
		}
	}
	...
	/**
	 * <p>
	 * Returns the Unicode character that the specified key would produce when
	 * the specified meta bits (see {@link MetaKeyKeyListener}) were active.
	 * </p>
	 * <p>
	 * Returns 0 if the key is not one that is used to type Unicode characters.
	 * </p>
	 * <p>
	 * If the return value has bit {@link #COMBINING_ACCENT} set, the key is a
	 * "dead key" that should be combined with another to actually produce a
	 * character -- see {@link #getDeadChar} -- after masking with
	 * {@link #COMBINING_ACCENT_MASK}.
	 * </p>
	 */
	public int get(int keyCode, int meta) {
		...
		if ((meta & MetaKeyKeyListener.META_CAP_LOCKED) != 0) {
			if (get_native(mPointer, keyCode, KeyEvent.META_SHIFT_ON) == get_native(
					mPointer, keyCode, KeyEvent.META_ALT_ON)) {
				meta &= ~KeyEvent.META_SHIFT_ON;
			}
		}
	...
	/**
	 * Gets the number or symbol associated with the key. The character value is
	 * returned, not the numeric value. If the key is not a number, but is a
	 * symbol, the symbol is retuned.
	 */
	...
	/**
	 * If one of the chars in the array can be generated by keyCode, return the
	 * char; otherwise return '\0'.
	 * 
	 * @param keyCode
	 *            the key code to look at
	 * @param chars
	 *            the characters to try to find
	 * @param modifiers
	 *            the modifier bits to prefer. If any of these bits are set, if
	 *            there are multiple choices, that could work, the one for this
	...
	/**
	 * Get the primary character for this key. In other words, the label that is
	...
	public static class KeyData {
		...
		public char number;
		/**
		 * The character that will be generated in various meta states (the same
		 * ones used for {@link #get} and defined as
	 ...
	 * 
	 * @param keyCode
	 *            the keyCode to look for
	 * @param results
	 *            a {@link KeyData} that will be filled with the results.
	 * 
	 ...
	 * plausibly generate the provided sequence of characters. It is not
	 * guaranteed that the sequence is the only way to generate these events or
	 * that it is optimal.
	 * 
	 * @return an array of KeyEvent objects, or null if the given char array can
	 *         not be generated using the current key character map.
	 */
	public KeyEvent[] getEvents(char[] chars) {
		...
		for (int i = 0; i < N; i++) {
			...
			int key = (int) (keys[i]);
			rv[index] = new KeyEvent(now, now, KeyEvent.ACTION_DOWN, key, 0,
					meta, device, 0);
			index++;
			rv[index] = new KeyEvent(now, now, KeyEvent.ACTION_UP, key, 0,
					meta, device, 0);
			index++;
	...
	/**
	 * Queries the framework about whether any physical keys exist on the device
	 * that are capable of producing the given key codes.
	 */
	...
	/**
	 * Maps Unicode combining diacritical to display-form dead key (display
	 * character shifted left 16 bits).
	 */
	private static SparseIntArray COMBINING = new SparseIntArray();

	/**
	 * Maps combinations of (display-form) dead key and second character to
	 * combined output character.
	 */
	private static SparseIntArray DEAD = new SparseIntArray();

	/*
	 * TODO: Change the table format to support full 21-bit-wide accent

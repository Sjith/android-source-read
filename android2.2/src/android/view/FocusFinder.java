 ...
 * <br>找到当前view视图树结构上的下一个foucusable view的一种算法。
 */
public class FocusFinder {
	...
	/**
	 * Find the next view to take focus in root's descendants, starting from the
	 * view that currently is focused.
	 * 
	 * @param root
	 *            Contains focused
	 * @param focused
	...
	public final View findNextFocus(ViewGroup root, View focused, int direction) {
		...
		if (focused != null) {
			...
			View userSetNextFocus = focused.findUserSetNextFocus(root,
					direction);
			if (userSetNextFocus != null
					&& userSetNextFocus.isFocusable()
					&& (!userSetNextFocus.isInTouchMode() || userSetNextFocus
	 ...
	 * @return The next focusable view, or null if none exists.
	 */
	public View findNextFocusFromRect(ViewGroup root, Rect focusedRect,
			int direction) {
	...
	private View findNextFocus(ViewGroup root, View focused, Rect focusedRect,
		...
		for (int i = 0; i < numFocusables; i++) {
			...
			if (isBetterCandidate(direction, focusedRect, mOtherRect,
					mBestCandidateRect)) {
	 ...
	 * 
	 * @return Whether rect1 is a better candidate than rect2 by virtue of it
	 *         being in src's beam
	 */
	boolean beamBeats(int direction, Rect source, Rect rect1, Rect rect2) {
		...
		// source
		// and rect2 is below, then we always prefer the in beam rect1, since
		// rect2
	 ...
	 * checks whether the dest is at least partially to the direction of (e.g
	 * left of) from source.
	 * 
	...
	/**
	 * Do the "beams" w.r.t the given direcition's axos of rect1 and rect2
	 * overlap?
	 * 
	 * @param direction
	 *            the direction (up, down, left, right)
	...
	/**
	 * @return The distance from the edge furthest in the given direction of
	 *         source to the edge nearest in the given direction of dest. If the
	 ...
	 *         edge of source to the far edge of dest. If the dest is not in the
	 *         direction from source, return 1 (to break ties with
	 *         {@link #majorAxisDistance}).
	 */
	static int majorAxisDistanceToFarEdge(int direction, Rect source, Rect dest) {
		return Math.max(1,
				majorAxisDistanceToFarEdgeRaw(direction, source, dest));
	}

	static int majorAxisDistanceToFarEdgeRaw(int direction, Rect source,
			Rect dest) {
		switch (direction) {
		case View.FOCUS_LEFT:
	...
	static int minorAxisDistance(int direction, Rect source, Rect dest) {
		...
		case View.FOCUS_RIGHT:
			...
			return Math
					.abs(((source.top + source.height() / 2) - ((dest.top + dest
							.height() / 2))));
		case View.FOCUS_UP:
		case View.FOCUS_DOWN:
			// the distance between the center horizontals
			return Math
					.abs(((source.left + source.width() / 2) - ((dest.left + dest
							.width() / 2))));
		}
		throw new IllegalArgumentException("direction must be one of "
				+ "{FOCUS_UP, FOCUS_DOWN, FOCUS_LEFT, FOCUS_RIGHT}.");
	 ...
	 *            Direction to look
	 * @param deltas
	 *            Offset from the <x, y> to the edge of the nearest view. Note
	 *            that this array may already be populated with values.
	 * @return The nearest touchable view, or null if none exists.
	 */
	public View findNearestTouchable(ViewGroup root, int x, int y,
			int direction, int[] deltas) {
		...
		for (int i = 0; i < numTouchables; i++) {
			...
			root.offsetRectBetweenParentAndChild(touchable, touchableBounds,
					true, true);
	...
	private boolean isTouchCandidate(int x, int y, Rect destRect, int direction) {
		switch (direction) {
		case View.FOCUS_LEFT:
			return destRect.left <= x && destRect.top <= y
					&& y <= destRect.bottom;
		case View.FOCUS_RIGHT:
			return destRect.left >= x && destRect.top <= y
					&& y <= destRect.bottom;
		case View.FOCUS_UP:
			return destRect.top <= y && destRect.left <= x
					&& x <= destRect.right;
		case View.FOCUS_DOWN:
			return destRect.top >= y && destRect.left <= x
					&& x <= destRect.right;

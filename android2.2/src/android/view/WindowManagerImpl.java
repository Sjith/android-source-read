...
/**
 * 与global window manager
 * 进行的低级别的通讯。实现了ViewManager接口，允许你添加View的一个子类作为屏幕的顶层window, 另外window
 * manager制定了布局参数用来控制window的显示方式。同时实现了WindowManager接口，允许用来控制与设备相关的display。 <br>
 * 通常不会直接使用WindowManager的实例，通常使用Activity和Dialog作为高级别的方法. <br>
 ...
 * parameters are defined for control over how windows are displayed. It also
 * implemens the WindowManager interface, allowing you to control the displays
 * attached to the device.
 * 
 * <p>
 * Applications will not normally use WindowManager directly, instead relying on
 * the higher-level facilities in {@link android.app.Activity} and
...
public class WindowManagerImpl implements WindowManager {
	/**
	 * The user is navigating with keys (not the touch screen), so navigational
	 * focus should be shown.
	 */
	...
	View removeViewLocked(int index) {
		...
		InputMethodManager imm = InputMethodManager.getInstance(view
				.getContext());
		if (imm != null) {
		...
		synchronized (this) {
			...
			for (int i = 0; i < count; i++) {
				...
				if (token == null || mParams[i].token == token) {
					...
					if (who != null) {
						WindowLeaked leak = new WindowLeaked(what + " " + who
								+ " has leaked window " + root.getView()

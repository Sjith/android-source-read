...
/**
 * 可以被应用到View，surface，或者其他对象的动画的抽象 <br>
 * Abstraction for an Animation that can be applied to Views, Surfaces, or other
 * objects. See the {@link android.view.animation animation package description
 * file}.
 */
public abstract class Animation implements Cloneable {
	/**
	 * Repeat the animation indefinitely. <br>
	 * 一直执行此动画，不停止
	 */
	...
	/**
	 * 指定维度(横向或纵向)使用绝对像素数 <br>
	 * The specified dimension is an absolute number of pixels.
	 */
	public static final int ABSOLUTE = 0;

	/**
	 * 指定维度（横向或纵向）使用一个float类型数据，该float值与将要执行动画的对象的宽度或高度相乘。 <br>
	...
	/**
	 * 指定维度（横向或纵向）使用一个float类型数据，该float值与将要执行动画的对象的父View的宽度或高度相乘。 <br>
	 * The specified dimension holds a float and should be multiplied by the
	 ...
	 * 是否已经结束动画
	 * <br>
	 * Set by {@link #getTransformation(long, Transformation)} when the
	 * animation ends.
	 */
	...
	/**
	 * Set by {@link #getTransformation(long, Transformation)} when the
	 * animation repeats in REVERSE mode.
	 */
	 ...
	 * 动画是否已经成功初始化，以准备执行
	 * <br>
	 * This value must be set to true by {@link #initialize(int, int, int, int)}
	 * . It indicates the animation was successfully initialized and can be
	 * played.
	 */
	...
	/**
	 * 在执行时间多少毫秒以后执行动画，如果该变量为正数的话则延迟指定时间
	 * <br>
	 ...
	 * 动画循环执行的次数
	 * <br>
	 * The number of times the animation must repeat. By default, an animation
	...
	/**
	 * 动画监控器，当动画开始，结束或者重复的时候通知此监控器
	 * <br>
	 * The animation listener to be notified when the animation starts, ends or
	 * repeats.
	 */
	...
	/**
	 * 创建一个新的动画，duration为0，默认的插值器，fillBefore设置为true，fillAfter设置为false
	 * <br>
	 * Creates a new animation with a duration of 0ms, the default interpolator,
	 * with fillBefore set to true and fillAfter set to false
	 */
	public Animation() {
		ensureInterpolator();
	}
	...
	public Animation(Context context, AttributeSet attrs) {
		...
		setDuration((long) a.getInt(
				com.android.internal.R.styleable.Animation_duration, 0));
		setStartOffset((long) a.getInt(
				com.android.internal.R.styleable.Animation_startOffset, 0));

		setFillEnabled(a.getBoolean(
				com.android.internal.R.styleable.Animation_fillEnabled,
				mFillEnabled));
		setFillBefore(a.getBoolean(
				com.android.internal.R.styleable.Animation_fillBefore,
				mFillBefore));
		setFillAfter(a.getBoolean(
				com.android.internal.R.styleable.Animation_fillAfter,
				mFillAfter));

		final int resID = a.getResourceId(
				com.android.internal.R.styleable.Animation_interpolator, 0);
		if (resID > 0) {
			setInterpolator(context, resID);
		}

		setRepeatCount(a.getInt(
				com.android.internal.R.styleable.Animation_repeatCount,
				mRepeatCount));
		setRepeatMode(a.getInt(
				com.android.internal.R.styleable.Animation_repeatMode, RESTART));

		setZAdjustment(a.getInt(
				com.android.internal.R.styleable.Animation_zAdjustment,
				ZORDER_NORMAL));

		setDetachWallpaper(a.getBoolean(
				com.android.internal.R.styleable.Animation_detachWallpaper,
				false));
	...
	/**
	 * 取消animation，如果动画监听器被设置则通知监听器结束动画,如果手动取消动画，再次执行动画的时候需要调用{@link #reset()}
	 * <br>
	 ...
	 * 
	 * <p>
	 * Objects that interpret a Animations should call this method when the
	 * sizes of the object being animated and its parent are known, and before
	 * calling {@link #getTransformation}.
	 * 
	 * 
	 * @param width
	...
	/**
	 * Sets the acceleration curve for this animation. The interpolator is
	 * loaded as a resource from the specified context.
	 * 
	 * @param context
	 *            The application environment
	 * @param resID
	 ...
	 * 
	 * @param startOffset
	 *            When this Animation should start, in milliseconds from the
	 *            start time of the root AnimationSet.
	 ...
	 * <var>durationMillis</var>. In addition to adjusting the duration itself,
	 * this ensures that the repeat count also will not make it run longer than
	 * the given time.
	 * 
	 * @param durationMillis
	 ...
	 * {@link #getTransformation(long, Transformation)} is invoked. The time
	 * passed to this method should be obtained by calling
	...
	/**
	 * If fillEnabled is true, this animation will apply fillBefore and
	 * fillAfter.
	 * 
	 * @return true if the animation will take fillBefore and fillAfter into
	 *         account
	 ...
	 * and fillAfter. Otherwise, fillBefore and fillAfter are ignored and the
	 * animation transformation is always applied.
	 * 
	 * @param fillEnabled
	 *            true if the animation should take fillBefore and fillAfter
	 *            into account
	 * @attr ref android.R.styleable#Animation_fillEnabled
	 * 
	 ...
	 * before the start time of the animation. Defaults to true if not set. Note
	 * that this applies when using an
	 * {@link android.view.animation.AnimationSet AnimationSet} to chain
	 * animations. The transformation is not applied before the AnimationSet
	 * itself starts.
	 * 
	 * @param fillBefore
	 *            true if the animation should apply its transformation before
	 *            it starts
	 ...
	 * will persist when it is finished. Defaults to false if not set. Note that
	 * this applies when using an {@link android.view.animation.AnimationSet
	 * AnimationSet} to chain animations. The transformation is not applied
	 * before the AnimationSet itself starts.
	 * 
	 * @param fillAfter
	 *            true if the animation should apply its transformation after it
	 *            ends
	 ...
	 * 
	 * @param zAdjustment
	 *            The desired mode, one of {@link #ZORDER_NORMAL},
	 ...
	 * 
	 * @param detachWallpaper
	 *            true if the wallpaper should be detached from the animation
	...
	/**
	 * If fillAfter is true, this animation will apply its transformation after
	 ...
	 * 
	 * @return Returns one of {@link #ZORDER_NORMAL}, {@link #ZORDER_TOP}, or
	 *         {@link #ZORDER_BOTTOM}.
	 * @attr ref android.R.styleable#Animation_zAdjustment
	 */
	 ...
	 * notified of animation events such as the end of the animation or the
	...
	/**
	 * Compute a hint at how long the entire animation may last, in
	 * milliseconds. Animations can be written to cause themselves to run for a
	 * different duration than what is computed here, but generally this should
	 * be accurate.
	 */
	 ...
	 * Implementations of this method should always replace the specified
	 * Transformation or document they are doing otherwise.
	 * 
	 * @param currentTime
	 *            Where we are in the animation. This is wall clock time.
	 * @param outTransformation
	 * 			  调用者提供的转换，被动画填充。
	 *            A tranformation object that is provided by the caller and will
	 *            be filled in by the animation.
	...
	/**
	 * getTransformation的辅助方法，子类需要实现本方法来执行已经给定了一个Interpolation的
	 * 转换。
	 * <br>
	 ...
	 * 			  时间的规范化值(0.0到1.0），在动画执行过整个插值器函数之后<br>
	...
	public void initializeInvalidateRegion(int left, int top, int right,
		...
		if (mFillBefore) {
			final Transformation previousTransformation = mPreviousTransformation;
			applyTransformation(mInterpolator.getInterpolation(0.0f),
	...
	protected static class Description {
		 ...
		 * <li>A size relative to the size of the object being animated. This is
		 * represented by a number followed by "%".</li> *
		 * <li>A size relative to the size of the parent of object being
		 * animated. This is represented by a number followed by "%p".</li>
		 * </ol>
		 * 
		 * @param value
		 *            The typed value to parse
	...
	/**
	 * 接收动画通知的动画监控器。动画通知表示动画的相关事件，比如动画的结束，重复。
	 * <br>
	 * <p>

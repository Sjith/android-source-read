...
/**
 * 让一个layout，VieGroup的所有child执行动画。每一个child执行一个相同的动画，但是在不同的事件开始。
 * 被ViewGroup使用用来计算每一个child在延迟多长时间后执行动画，通过每个child的字母顺序就像他们在 ViewGroup中的索引来计算延迟。
 * 标准的计算延迟的方法是将他们在view group中的索引乘以固定的值。子类可以重载
 * {@link #getDelayForView(android.view.View)}来设定一个不同的算法。例如
 * {@link android.view.animation.GridLayoutAnimationController} 使用孩子所在的行号和列号进行计算<br>
 ...
 * are supposed to override {@link #getDelayForView(android.view.View)} to
 * implement a different way of computing the delay. For instance, a
 ...
 * group.
 * 
 * Information used to compute the animation delay of each child are stored in
 * an instance of
...
public class LayoutAnimationController {
	/**
	 * Distributes the animation delays in the order in which view were added to
	 * their view group. <br>按照正常顺序来执行动画
	 */
	public static final int ORDER_NORMAL = 0;

	/**
	 * Distributes the animation delays in the reverse order in which view were
	 * added to their view group.<br>按照逆序来执行动画
	 */
	 ...
	 * 
	 * @param context
	 *            the Context the view group is running in, through which it can
	 *            access the resources
	 * @param attrs
	 *            the attributes of the XML tag that is inflating the layout
	 *            animation controller
	 */
	public LayoutAnimationController(Context context, AttributeSet attrs) {
		TypedArray a = context.obtainStyledAttributes(attrs,
				com.android.internal.R.styleable.LayoutAnimation);

		Animation.Description d = Animation.Description
				.parseValue(a
						.peekValue(com.android.internal.R.styleable.LayoutAnimation_delay));
		mDelay = d.value;

		mOrder = a
				.getInt(com.android.internal.R.styleable.LayoutAnimation_animationOrder,
						ORDER_NORMAL);

		int resource = a.getResourceId(
				com.android.internal.R.styleable.LayoutAnimation_animation, 0);
		if (resource > 0) {
			setAnimation(context, resource);
		}

		resource = a.getResourceId(
				com.android.internal.R.styleable.LayoutAnimation_interpolator,
				0);
		if (resource > 0) {
	...
	/**
	 * Creates a new layout animation controller with a delay of 50% and the
	 * specified animation.
	 * 
	 * @param animation
	...
	/**
	 * Creates a new layout animation controller with the specified delay and
	 ...
	 * 
	 * @param order
	 *            one of {@link #ORDER_NORMAL}, {@link #ORDER_REVERSE} or
	 *            {@link #ORDER_RANDOM}
	 * 
	 * @attr ref android.R.styleable#LayoutAnimation_animationOrder
	 */
	...
	/**
	 * Returns the delay by which the children's animation are offset. The delay
	 * is expressed as a fraction of the animation duration.
	 * 
	 ...
	 * 
	 * @return true if all of the children's animations are over, false
	 *         otherwise
	 */
	public boolean isDone() {
		return AnimationUtils.currentAnimationTimeMillis() > mAnimation
	...
	/**
	 * 每一个view使用的参数集合，这个view在执行动画的view group中，这些参数用来计算view的执行动画时间。 <br>
	...
	public static class AnimationParameters {
		 ...
		 * these parameters are attached. <br>
		 * 所在view group的child个数
		 */
		public int count;

		/**
		 * The index of the view to which these parameters are attached in its
		 * containing view group. <br>
		 * 在所在view group的索引号
		 */
		public int index;
	}

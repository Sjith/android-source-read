...
public abstract class Window {
	...
	/**
	 * Flag for asking for an OpenGL enabled window. All 2D graphics will be
	 * handled by OpenGL ES.
	 * 
	 * @hide
	 */
	...
	/**
	 * 从window向window的使用这进行回调的api。允许客户端解释key分发，panel和menu操作。 API from a Window
	 * back to its caller. This allows the client to intercept key dispatching,
	 * panels and menus, etc.
	 */
	...
	/**
	 * Set the container for this window. If not set, the DecorWindow operates
	 * as a top-level window; otherwise, it negotiates with the container to
	 ...
	 * 
	 * @return Window The containing window, or null if this is a top-level
	 *         window.
	 */
	...
	/**
	 * Set the window manager for use by this Window to, for example, display
	 * panels. This is <em>not</em> used for displaying the Window itself --
	 * that must be done by the client.
	 * 
	 * @param wm
	 ...
	 * (based on the {@link android.R.attr#windowIsFloating} attribute in the
	 * style/theme).
	 * 
	 ...
	 * the default format that is selected by the Window based on its window
	 * decorations.
	 * 
	 * @param format
	 *            The new window format (see PixelFormat). Use
	 *            PixelFormat.UNKNOWN to allow the Window to select the format.
	 * 
	 * @see PixelFormat
	 */
	 ...
	 * 0 here will override the animations the window would normally retrieve
	 * from its theme.
	 */
	 ...
	 * "unspecified" here will override the input mode the window would normally
	 * retrieve from its theme.
	 */
	 ...
	 * {@link WindowManager.LayoutParams#FLAG_LAYOUT_INSET_DECOR}. These will be
	 * set for you based on the {@link android.R.attr#windowIsFloating}
	 * attribute.
	 * 
	 * @param flags
	 ...
	 * 
	 * @return WindowManager.LayoutParams Either the existing window attributes
	 *         object, or a freshly created one if there is none.
	 */
	 ...
	 * setContentView(). May be called as many times as desired as long as it is
	 * before setContentView(). If not called, no extended features will be
	 * available. You can not turn off a feature once it is requested. You canot
	 * use other title features with {@link #FEATURE_CUSTOM_TITLE}.
	 * 
	 * @param featureId
	...
	public boolean requestFeature(int featureId) {
		...
		mFeatures |= flag;
		mLocalFeatures |= mContainer != null ? (flag & ~mContainer.mFeatures)
				: flag;
	 ...
	 * implicitly call {@link #getDecorView} for you, with all of the associated
	 * side-effects.
	 * 
	...
	/**
	 * Convenience for
	 * {@link #setContentView(View, android.view.ViewGroup.LayoutParams)} set
	 * the screen content to an explicit view. This view is placed directly into
	 ...
	 * window that can not, from this point forward, be changed: the features
	 * that have been requested with {@link #requestFeature(int)}, and certain
	 * window flags as described in {@link #setFlags(int, int)}.
	 * 
	 * @param view
	 *            The desired content to display.
	 * @param params
	...
	/**
	 * Variation on
	 * {@link #setContentView(View, android.view.ViewGroup.LayoutParams)} to add
	 * an additional content view to the screen. Added after any existing ones
	 * in the screen -- existing views are NOT removed.
	 * 
	 * @param view
	 *            The desired content to display.
	 * @param params
	 ...
	 * are none. Note that this does not look in any containing Window.
	 * 
	...
	public abstract boolean performPanelShortcut(int featureId, int keyCode,
			KeyEvent event, int flags);

	public abstract boolean performPanelIdentifierAction(int featureId, int id,
			int flags);
	 ...
	 * 
	 * @param drawable
	 *            The new Drawable to use for this window's background.
	 */
	 ...
	 * 
	 * @param featureId
	 *            The desired drawable feature to change, defined as a constant
	 *            by Window.
	 * @param resId
	...
	/**
	 * Set the value for a drawable feature of this window, from a URI. You must
	 ...
	 * 
	 * @param featureId
	 *            The desired drawable feature to change. Features are constants
	 *            defined by Window.
	 * @param uri
	 *            The desired URI.
	 */
	...
	/**
	 * Set an explicit Drawable value for feature of this window. You must have
	 ...
	 * 
	 * @param featureId
	 *            The desired feature to change. Features are constants defined
	 *            by Window.
	 * @param value
	 *            The value for the feature. The interpretation of this value is
	 *            feature-specific.
	 */
	 ...
	 * further down the view hierarchy. Application developers should not need
	 * to implement or call this.
	 * 
	 */
	 ...
	 * 
	 * <p>
	 * <em>Note that calling this function for the first time "locks in"
	 * various window characteristics as described in
	 * {@link #setContentView(View, android.view.ViewGroup.LayoutParams)}.</em>
	 * </p>
	 * 
	 ...
	 * that were given to requestFeature(), and are being handled by this Window
	 * itself or its container. That is, it is the set of requested features
	...
	/**
	 * Return the feature bits that are being implemented by this Window. This
	 * is the set of features that were given to requestFeature(), and are being
	 * handled by only this Window itself, not by its containers.
	 * 
	 * @return int The feature bits.
	 */
	...
	/**
	 * Set the default format of window, as per the PixelFormat types. This is
	 * the format that will be used unless the client specifies in explicit
	 ...
	 * 
	 * @param keyCode
	 *            the key code from {@link android.view.KeyEvent} to check.
	 * @param event
	 *            the {@link android.view.KeyEvent} to use to help check.
	 */

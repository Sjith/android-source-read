...
/**
 * 该类用于将layout的xml文件转为View对象,不能直接使用该类，一般情况下需要
 * {@link android.app.Activity#getLayoutInflater()} or
 * {@link Context#getSystemService} 来获取该类。 This class is used to instantiate
 * layout XML file into its corresponding View objects. It is never be used
 * directly -- use {@link android.app.Activity#getLayoutInflater()} or
		...
		synchronized (mConstructorArgs) {
			...
			try {
				...
				if (TAG_MERGE.equals(name)) {
					...
					// We are supposed to attach all the views we found (int
					// temp)
					// to root. Do that now.
					if (root != null && attachToRoot) {
				...
				try {
					...
					if (TAG_MERGE.equals(childName)) {
						...
						int id = a.getResourceId(
								com.android.internal.R.styleable.View_id,
								View.NO_ID);
						// While we're at it, let's try to override
						// android:visibility.
						int visibility = a
								.getInt(com.android.internal.R.styleable.View_visibility,
										-1);
						a.recycle();

...
/**
 * 该类描述了所有的设备配置信息。这些信息会影响应用程序资源的抽取，包含了用于自定义的配置选项 和不同设备之间的选项 <br>
 ...
 * configuration options (locale and scaling) as well as dynamic device
 * configuration (various types of input devices).
 */
public final class Configuration implements Parcelable,
		Comparable<Configuration> {
	/**
	 * Current user preference for the scaling factor for fonts, relative to the
	 * base density scaling.
	 */
	...
	/**
	 * Special flag we generate to indicate that the screen layout requires us
	 * to use a compatibility mode for apps that are not modern layout aware.
	 * 
	 * @hide
	 */
	 ...
	 * <p>
	 * The {@link #SCREENLAYOUT_SIZE_MASK} bits define the overall size of the
	 * screen. They may be one of {@link #SCREENLAYOUT_SIZE_SMALL},
	 * {@link #SCREENLAYOUT_SIZE_NORMAL}, or {@link #SCREENLAYOUT_SIZE_LARGE}.
	 * 
	 * <p>
	 * The {@link #SCREENLAYOUT_LONG_MASK} defines whether the screen is
	 ...
	 * {@link #TOUCHSCREEN_NOTOUCH}, {@link #TOUCHSCREEN_STYLUS},
	 ...
	 * {@link #KEYBOARD_NOKEYS}, {@link #KEYBOARD_QWERTY},
	 * {@link #KEYBOARD_12KEY}.
	 */
	public int keyboard;
	...
	/**
	 * A flag indicating whether the hard keyboard has been hidden. This will be
	 * set on a device with a mechanism to hide the keyboard from the user, when
	 * that mechanism is closed. One of: {@link #HARDKEYBOARDHIDDEN_NO},
	 * {@link #HARDKEYBOARDHIDDEN_YES}.
	 */
	 ...
	 * {@link #NAVIGATION_NONAV}, {@link #NAVIGATION_DPAD},
	...
	/**
	 * A flag indicating whether any 5-way or DPAD navigation available. This
	 * will be set on a device with a mechanism to hide the navigation controls
	 * from the user, when that mechanism is closed. One of:
	 ...
	 * {@link #ORIENTATION_LANDSCAPE}, {@link #ORIENTATION_PORTRAIT}, or
	 * {@link #ORIENTATION_SQUARE}.
	 */
	 ...
	 * {@link #UI_MODE_TYPE_NORMAL}, {@link #UI_MODE_TYPE_DESK}, or
	 * {@link #UI_MODE_TYPE_CAR}.
	 * 
	 * <p>
	 * The {@link #UI_MODE_NIGHT_MASK} defines whether the screen is in a
	 * special mode. They may be one of {@link #UI_MODE_NIGHT_UNDEFINED},
	...
	/**
	 * Copy the fields from delta into this Configuration object, keeping track
	 * of which ones have changed. Any undefined fields in <var>delta</var> are
	 * ignored and not copied in to the current Configuration.
	 * 
	 * @return Returns a bit mask of the changed fields, as per {@link #diff}.
	 */
	public int updateFrom(Configuration delta) {
		...
		if (delta.locale != null
			...
			locale = delta.locale != null ? (Locale) delta.locale.clone()
					: null;
		}
		if (delta.userSetLocale
				&& (!userSetLocale || ((changed & ActivityInfo.CONFIG_LOCALE) != 0))) {
			userSetLocale = true;
			changed |= ActivityInfo.CONFIG_LOCALE;
		}
	 ...
	 * and the given one. Does not change the values of either. Any undefined
	 * fields in <var>delta</var> are ignored.
	 * 
	 * @return Returns a bit mask indicating which configuration values has
	 *         changed, containing any combination of
	 ...
	 *            The configuration changes that the resource can handled, as
	 *            given in
	 ...
	 *       that 'this' is your current sequence and 'other' is a new one you
	 *       have received some how and want to compare with what you have.
	 */
	...
	public int hashCode() {
		return ((int) this.fontScale) + this.mcc + this.mnc
				...
				+ this.touchscreen + this.keyboard + this.keyboardHidden
				+ this.hardKeyboardHidden + this.navigation
				+ this.navigationHidden + this.orientation + this.screenLayout
				+ this.uiMode;
	}
}

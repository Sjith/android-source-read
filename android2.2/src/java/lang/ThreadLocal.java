...
/**
 * 实现一个tread-local存储，也就是每一个线程都有其对应的一个变量，所有的线程共享一个{@code ThreadLocal}变量,
 * 但是访问的时候会返回不同的值，一个线程的更改不会影响到其他线程。支持{@code null}值。其实就是通过一个map来进行线程的映射。<br>
 * Implements a thread-local storage, that is, a variable for which each thread
 * has its own value. All threads share the same {@code ThreadLocal} object, but
 * each sees a different value when accessing it, and changes made by one thread
 * do not affect the other threads. The implementation supports {@code null}
 * values.
 * 
...
public class ThreadLocal<T> {
	 ...
	 * is followed by a {@link #get()} before a {@link #set}, {@code #get()}
	...
	private final Reference<ThreadLocal<T>> reference = new WeakReference<ThreadLocal<T>>(
			this);
	...
	/**
	 * Per-thread map of ThreadLocal instances to values. <br>
	 * 每一个线程映射成ThreadLocal对象的值
	 */
	static class Values {
		...
		/**
		 * Rehashes the table, expanding or contracting it as necessary. Gets
		 * rid of tombstones. Returns true if a rehash occurred. We must rehash
		 * every time we fill a null slot; we depend on the presence of null
		 * slots to end searches (otherwise, we'll infinitely loop).
		 */
